<div class="form-group row">
    <label for="settings-options" class="col-2 col-form-label">{{trans('navbar::field.options.label')}}</label>
    <div class="col-8">
        <div class="settings-options form-control">
            <table class="table table-nowrap table-sortable table-hover">
                ...
                <tr>
                    ...
                    <td>{{trans('navbar::field.options.text')}}</td>
                    <td width="40%">{{trans('navbar::field.options.value')}}</td>
                    <td width="10%"></td>
                </tr>
                </thead>
                <tbody>
            ...
            <script type="text/template" class="settings-options-template">
                <tr>
                    <td class="drag"></td>
                    <td>
                        <input type="text" class="form-control required" name="settings[options][:index][text]"
                               value=":text" placeholder="{{trans('navbar::field.options.text')}}">
                    </td>
                    <td>
                        <input type="text" class="form-control required" name="settings[options][:index][value]"
                               value=":value" placeholder="{{trans('navbar::field.options.value')}}">
                    </td>
                    <td class="text-center"><a href="javascript:;" class="settings-options-delete"> <i
...
<script type="text/javascript">
    $(function () {
        ...
        var item = {text: '', value: ''};
        var data = @json($field->settings->options ?? $type->settings->options ?? null) ||
        [item, item];
