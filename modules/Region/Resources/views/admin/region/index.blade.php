            ...
            <nav class="breadcrumb mr-auto">
                ...
                @if($id && $parents = $region->parents)
                    @foreach($parents as $p)
                        <a class="breadcrumb-item" href="{{route('region.index', $p->id)}}">
                            {{$p->title}}
                        </a>
                    @endforeach
                @endif
                        ...
                        <tr class="item {{$region->disabled ? 'disabled' : 'active'}}">
                            ...
                            <td>
                                ...
                                <div class="manage">
                                    ...
                                    <a href="javascript:;" data-url="{{route('region.edit', $region->id)}}"
                                       data-width="800"
                            ...
                            <td>
                                @if($region->disabled)
                                    <i class="fa fa-times-circle fa-2x text-muted"
                                       title="{{trans('master.disabled')}}"></i>
                                @else
                                    <i class="fa fa-check-circle fa-2x text-success"
                                       title="{{trans('master.actived')}}"></i>
                                @endif
                            </td>
                        </tr>
                    @endforeach
