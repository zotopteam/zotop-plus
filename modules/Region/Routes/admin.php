...
$router->group(['prefix' =>'/region','module'=>'region'], function (Router $router) {
    ...
    $router->post('/disable/{id}', 'IndexController@disable')->name('region.disable')->middleware('allow:region.disable');

    $router->post('/enable/{id}', 'IndexController@enable')->name('region.enable')->middleware('allow:region.enable');
});
