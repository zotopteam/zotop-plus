...
$router->group(['prefix' => 'media'], function (Router $router) {
    $router->get('index/{folder_id?}', 'MediaController@index')->name('media.index');
    $router->get('type/{type}', 'MediaController@type')->name('media.type');
    $router->get('show/{id}', 'MediaController@show')->name('media.show');
