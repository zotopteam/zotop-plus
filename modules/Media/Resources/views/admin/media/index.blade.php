...
<div class="main">
    <div class="main-header">
        ...
        <div class="main-action">
            ...
            <div class="input-group">
                <input name="keywords" value="{{$keywords}}" class="form-control border-primary bg-light" type="search"
                    placeholder="{{trans('media::media.search.placeholder')}}" required="required" aria-label="Search">
    ...
    <div class="main-body scrollable" id="file-upload-dragdrop">
        ...
        <table class="table table-nowrap table-hover checkable">
            ...
            <tbody>
                @foreach($media as $m)
                <tr class="checkable-item js-media-open" data-type="{{$m->type}}" data-link="{{$m->link}}"
                    data-title="{{$m->name}}">
                    <td class="select">
                        @if ($m->isFolder())
                        <input type="checkbox" name="ids[]" value="{{$m->id}}" data-type="folder"
                            class="checkable-checkbox text-muted">
                        @else
                        <input type="checkbox" name="ids[]" value="{{$m->id}}" data-type="file"
                            class="checkable-checkbox text-muted">
                        @endif
                    </td>
                    <td width="1%" class="text-center pr-2">
                        ...
                        @elseif ($m->isImage())
                        <div class="icon icon-md"><img src="{{preview($m->disk.':'.$m->path, 300)}}"></div>
                        @else
                    ...
                    <td width="10%" class="manage manage-hover text-right">
                        ...
                        @if ($m->isImage())
                        <a href="javascript:;" class="manage-item js-image" data-url="{{$m->link}}"
                            data-title="{{$m->name}}">
                        ...
                        @if ($m->isFolder())
                        <a class="manage-item js-prompt" href="javascript:;"
                            data-url="{{route('media.rename',[$m->id])}}" data-prompt="{{trans('core::folder.name')}}"
                        ...
                        <a href="javascript:;" class="manage-item js-move" data-id="{{$m->id}}"
                            data-title="{{$m->name}}">
                    ...
                    <td>
                        ...
                        <span class="font-weight-light text-sm created_at" title="{{$m->created_at}}"
                            data-toggle="tooltip">{{$m->created_at_human}}</span>
                    </td>
                </tr>
                @endforeach
    ...
    <div class="main-footer">
        <div class="main-action mr-auto">
            <div class="btn-group dropup">
                ...
                <button type="button" class="btn btn-light dropdown-toggle dropdown-toggle-icon" data-toggle="dropdown"
                    aria-haspopup="true" aria-expanded="false">
                    <i class="fa fa-ellipsis-v"></i>
                </button>
                <div class="dropdown-menu dropdown-menu-left">
                    <a href="javascript:;" class="dropdown-item js-check-none">{{trans('media::media.select.none')}}</a>
                    <a href="javascript:;" class="dropdown-item js-check"
                        data-type="folder">{{trans('media::media.select.folder')}}</a>
                    <a href="javascript:;" class="dropdown-item js-check"
                        data-type="file">{{trans('media::media.select.file')}}</a>
                </div>
            </div>

            <button type="button" class="btn btn-success checkable-operator disabled" disabled="disabled"
                data-operate="move">
                <i class="fa fa-arrows-alt fa-fw"></i> {{trans('master.move')}}
            </button>
            <button type="button" class="btn btn-danger checkable-operator disabled" disabled="disabled"
                data-operate="delete">
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $(document).on('dblclick', '.js-media-open', function(event) {
            ...
            if (type == 'image') {
                $.image(link, title).statusbar(info);
            }
