    <div class="form-group row">
        ...
        <div class="col-4">
            <div class="input-group">
                ...
                <z-field type="number" name="settings[min]" value="$field->settings->min ?? $type->settings->min ?? ''" min="$type->settings->min ?? null"/>
                <div class="input-group-prepend"><span class="input-group-text">{{trans('content::field.fileslength.max')}}</span></div>
                <z-field type="number" name="settings[max]" value="$field->settings->max ?? $type->settings->max ?? ''" min="$type->settings->min ?? null" max="$type->settings->max ?? null"/>
