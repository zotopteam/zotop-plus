...
$router->group(['prefix'=>'content', 'module'=>'content'], function (Router $router) {
    ...
    $router->group(['prefix' =>'field'], function (Router $router) {
        $router->get('index','FieldController@index')->name('content.field.index')->middleware('allow:content.field.index');
        $router->get('create','FieldController@create')->name('content.field.create')->middleware('allow:content.field.create');
        $router->post('store','FieldController@store')->name('content.field.store')->middleware('allow:content.field.store');
        $router->get('show/{id}','FieldController@show')->name('content.field.show')->middleware('allow:content.field.show');
        $router->get('edit/{id}','FieldController@edit')->name('content.field.edit')->middleware('allow:content.field.edit');
        $router->put('update/{id}','FieldController@update')->name('content.field.update')->middleware('allow:content.field.update');
        $router->delete('destroy/{id}','FieldController@destroy')->name('content.field.destroy')->middleware('allow:content.field.destroy');
    });
