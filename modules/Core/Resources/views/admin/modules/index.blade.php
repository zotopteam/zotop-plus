                ...
                <tr class="item {{$module->active?'active':'disabled'}}">
                    ...
                    <td class="pl-2">
                        ...
                        <div class="manage">
                            @if($module->installed)
                                @if($module->active)
                                <a href="javascript:;" data-url="{{route('core.modules.disable',[$name])}}" class="manage-item js-confirm">
                                    <i class="fa fa-times-circle fa-fw"></i> {{trans('core::master.disable')}}
                                </a>
                                @else
                                <a href="javascript:;" data-url="{{route('core.modules.enable',[$name])}}" class="manage-item js-confirm">
                                    <i class="fa fa-check-circle fa-fw"></i> {{trans('core::master.active')}}
                                </a>
                                @endif

                                @foreach(filter::fire('module.manage', [], $name) as $s)
                                <a href="{{$s['href']}}" class="manage-item {{$s['class']}}">
                                    <i class="{{$s['icon']}} fa-fw"></i>{{$s['text']}}
                                </a>
                                @endforeach
                           
                                <a href="javascript:;" data-url="{{route('core.modules.uninstall',[$name])}}" data-confirm="{{trans('core::modules.uninstall.confirm',[$module->title])}}"  class="manage-item js-confirm">
                                    <i class="fa fa-trash fa-fw"></i> {{trans('core::modules.uninstall')}}
                                </a>
                            @else
                                <a href="javascript:;" data-url="{{route('core.modules.install',[$name])}}" class="manage-item js-confirm">
                                    <i class="fa fa-wrench fa-fw"></i> {{trans('core::modules.install')}}
                                </a>
                                <a href="javascript:;" data-url="{{route('core.modules.delete',[$name])}}" data-confirm="{{trans('core::modules.delete.confirm',[$module->title])}}" class="manage-item js-confirm">
