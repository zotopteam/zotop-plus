            ...
            <div class="main-action">
                <z-form route="core.log.index" class="form-inline form-search" method="get">
                    <div class="input-group">
                        <input name="keywords" value="{{request('keywords')}}" class="form-control border-primary"
                               type="search"
                               placeholder="{{trans('master.keywords.placeholder')}}" required="required"
                               aria-label="Search">
                        <div class="input-group-append">
                            <button class="btn btn-primary" type="submit"><i class="fa fa-fw fa-search"></i></button>
                            @if (request('keywords'))
                                <a href="{{route('core.log.index')}}" class="btn btn-danger"> <i
                ...
                <table class="table table-nowrap table-hover">
                    ...
                    @foreach ($logs as $log)
                        ...
                        <span data-toggle="tooltip"
                              title="{{trans('core::log.user_id.label')}}: {{$log->user_id}}">{{$log->user->username}}</span>
                            </td>
                            <td>
                                <div class="m-0 p-0">
                            <span data-toggle="tooltip"
                                  title="{{trans('core::log.module.label')}}">{{$log->module}}</span>
                                    <span class="text-black-50">></span>
                                    <span data-toggle="tooltip"
                                          title="{{trans('core::log.controller.label')}}">{{$log->controller}}</span>
                                    <span class="text-black-50">></span>
                                    <span data-toggle="tooltip"
                                          title="{{trans('core::log.action.label')}}">{{$log->action}}</span>
                                </div>
