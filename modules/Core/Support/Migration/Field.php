...
class Field {
	...
	public function get($table)
	{
		...
		if ($columns) {
			$index   = new Index($table, $this->schema);
			$fields  = $this->getFields($columns, $index);
			$fields  = $this->setEnum($fields, $table);
			$indexes = $this->getMultiFieldIndexes($index);
	 ...
	 * @return array
	 */
	protected function getFields($columns, Index $Index)
	{
		$fields = array();
	 ...
	 * @return array
	 */
	protected function getMultiFieldIndexes(Index $Index)
	{
		$indexes = array();
		foreach ($Index->getMultiFieldIndexes() as $index) {
			$indexArray = [
				'field' => $index->columns,
				'type' => $index->type,
			];
