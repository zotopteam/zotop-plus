...
class ListActionCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'hook:list-action
                            {hook : The hook name of the action.}';
     ...
     * @var string
     */
    protected $description = 'Displays the list of specific action hook';
    ...
    private function showHookList(string $hook)
    {
        ...
        // 获取
        $listeners = $this->laravel['hook.action']->listeners($hook)->transform(function ($listener) use (&$number) {
